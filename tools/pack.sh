...
# Pack binary
echo Packing binary...
cd "../Uname.NET/bin/$releaseconf/netstandard2.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
cd "../Uname.NET.Console/bin/$releaseconf/net6.0/" && "$zippath" -r /tmp/$version-bin.zip . && cd -
if [ ! $? == 0 ]; then
